...
import 'package:saccoapp/dialogs/creditdialog.dart';



class CreditAccepted extends StatefulWidget {
  final String legalName;
  final String id;
  final String dob;
  CreditAccepted(this.legalName, this.id, this.dob);
  @override
  State<CreditAccepted> createState() => _CreditAcceptedState();
}

class _CreditAcceptedState extends State<CreditAccepted> {
  double progress = 0.0;
  File? file;
  UploadTask? task;
  final _dontre = GlobalKey<FormState>();
  checkFields() {
    final form = _dontre.currentState;
    if (form!.validate()) {
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            GestureDetector(
                onTap: () async {
                  CreditHandler().creditDialog(context);
                },
                child: Container(
                  height: 70,
                  width: size.width * 0.8,
